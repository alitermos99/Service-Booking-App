...
import { buildISODateTime, formatBookingDate, formatTo2Decimals, TODAY } from '@/app/utils/dateUtils';
                ...
                <div className="glass2 rounded-2xl p-5 space-y-4 sticky top-24">
                    ...
                    <div className="flex items-center gap-3 pb-4 border-b border-[rgba(255,255,255,0.07)]">
                        <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" 
                            style={{ background: data?.service?.iconBg ?? DEFAULT_ICON_BG }}
                        >
                            <span className="text-2xl">{ data?.service?.icon }</span>
                        </div>

                        <div>
                            <p className="text-sm font-semibold text-tx">{ data?.service?.title }</p>
                            <p className="text-xs text-muted">{ data?.service?.admin_id?.name }</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2 text-sm">
                        <div className="flex items-center gap-2 text-muted">
                            ...
                            <span>{ data?.service?.duration } minutes</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-muted" id="summaryDate">
                            ...
                            <span>{ formatBookingDate(buildISODateTime(TODAY, startTime)) }</span>
                        </div>
                    ...
                    <div className="pt-3 border-t space-y-2 border-[rgba(255,255,255,0.07)]">
                        ...
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="text-good font-semibold">$20.00</span>
                        </div>
                    </div>
